...
const todayBangla = document.getElementById('today-bangla');
const todayHijri = document.getElementById('today-hijri');
const todayWidgetTitle = document.getElementById('today-widget-title');
const bottomCtaTitle = document.getElementById('bottom-cta-title');
const bottomCtaDesc = document.getElementById('bottom-cta-desc');
const bottomCtaBtnText = document.getElementById('bottom-cta-btn-text');
...
function updateUIStaticText() {
  ...
  if (langToggleBtn) langToggleBtn.title = t.langToggle;
  if (themeToggleBtn) themeToggleBtn.title = t.themeToggle;

  document.getElementById('holiday-list-title').textContent = t.holidayList;
  todayWidgetTitle.textContent = t.todayLabel;
  bottomCtaTitle.textContent = t.bottomCtaTitle;
...
function updateTodayWidget() {
  ...
  const gregorianStr = `${formatNum(today.getDate())} ${t.months[today.getMonth()]} ${formatNum(today.getFullYear())} ${t.gregorianSuffix}`.trim();
  todayGregorian.textContent = gregorianStr;

  const bngDate = getBanglaDateObj(today);
  const banglaStr = `${formatNum(bngDate.day)} ${t.bMonths[bngDate.monthIndex]} ${formatNum(bngDate.year)} ${t.banglaSuffix}`.trim();
  todayBangla.textContent = banglaStr;

  const hjrDate = getHijriDateObj(today);
  const hijriStr = `${formatNum(hjrDate.day)} ${t.hMonths[hjrDate.monthIndex]} ${formatNum(hjrDate.year)} ${t.hijriSuffix}`.trim();
  todayHijri.textContent = hijriStr;
}
...
async function renderCalendar() {
  ...
  // Update Header Text and Nav Buttons
  ...
  prevBtn.querySelector('.prev-txt').textContent = t.months[prevMonth];
  nextBtn.querySelector('.next-txt').textContent = t.months[nextMonth];
  ...
  monthYearSub.innerHTML = `<span class="bng-month">${bngStr}</span> | <span class="hjr-month">${hjrStr}</span>`;
  ...
  // Fill empty slots before 1st day (previous month dates)
  for (let i = 0; i < firstDay; i++) {
    ...
    const bngDate = getBanglaDateObj(prevMonthDate);
    const hjrDate = getHijriDateObj(prevMonthDate);
  ...
  for (let d = 1; d <= daysInMonth; d++) {
    ...
    // Add custom tooltip if holiday
    if (holidayNames.length > 0) {
      html += `<div class="custom-tooltip">${holidayNames.join(', ')}</div>`;
    }
  ...
  for (let i = 0; i < remainingCells; i++) {
    ...
    const bngDate = getBanglaDateObj(nextMonthDate);
    const hjrDate = getHijriDateObj(nextMonthDate);
  ...
  if (!manifestCache) {
    holidayListContainer.innerHTML = `<li class="holiday-list-item"><span style="color:#d32f2f;">Failed to load holidays.</span></li>`;
    ...
    currentMonthHolidays.forEach((h) => {
      ...
      dateCol.className = 'holiday-date-col';
      ...
      contentCol.className = 'holiday-content-col';
